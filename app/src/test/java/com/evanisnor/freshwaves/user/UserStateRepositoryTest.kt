...
class UserStateRepositoryTest {
  @Test
  fun `currentState - when no user logged in - emits NoUser`() =
    runTest {
  ...
  @Test
  fun `currentState - when user becomes authorized - emits LoggedIn`() =
    runTest {
      val spotifyAuthorization =
  ...
  @Test
  fun `currentState - when user logs out - emits NoUser`() =
    runTest {
      val spotifyAuthorization =
        SpotifyAuthorizationImpl(
          FakeHandyAuth().apply {
  ...
  @Test
  fun `currentState - when user is logged in - emits LoggedIn`() =
    runTest {
      val spotifyAuthorization =
        SpotifyAuthorizationImpl(
          FakeHandyAuth().apply {
